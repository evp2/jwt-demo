...
public class RegisterService {
  ...
  public void registerUser(RegisterRequest registerRequest) {
    assert !registerRequest.username().isEmpty() && !registerRequest.email().isEmpty() && !registerRequest.password().isEmpty();
    if (userDetailsManager.userExists(registerRequest.username())) {
      LOG.warn("User: {} already exists", registerRequest.username());
      throw new RuntimeException("User already exists");
    }
    UserDetails user = User.withUsername(registerRequest.username())
